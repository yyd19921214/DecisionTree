...
public class DecisionTree {
	...
	public void init(Object[] dataArray, int index) {
		...
		/**
		 * 设置预测属性列为visable状态为true,其他属性为false
		 */
	...
	public void createDecisionTree(Object[] array) {
		Object[] maxgain = getMaxGainRatio(array,"ALL");//获取基尼比例最大的属性
		if (root == null) {
	...
	public Object[] getMaxGainRatio(Object[] array,String processingData) {
		...
		if (index >= 0) {
			...
			process_buffer.append("计算得到MaxGainRatio:" + gain + ",生成以"+processingData+"为分支的第"+(++countNode)+"个结点"+this.columnHeaderArray[preNodeIndex]+":"+trueattr+"\n");
			process_buffer.append("*************************************************结点分割线***************************************\n");
		}
		
		// TODO 抛弃不能进行预测的情况，记所以的GainRatio均小于0
		if (index != -1) {
			visable[index] = true;//此时该属性已经进行划分，以后不再参与划分
		}
		return result;
	}
